...
import { SafeAreaView,StyleSheet,ScrollView,Text,View, Dimensions ,Image} from "react-native";
...
const SlideShow = () => {
    const [imgActite, setimgActite] = useState(0);

    onchange = (naviteEvent) => {
        if(naviteEvent){
            const silde = Math.ceil(naviteEvent.contentOffset.x / naviteEvent.layoutMeasurement.width);
            if(silde != imgActite){
                setimgActite(silde);
            }
        }
                ...
                <View style={styles.wrapDot}>
                    {
                        ...
                        <Text 
                            key={e} 
                            style={imgActite == index ? styles.dotAcvite : styles.dot}>
                                ●
                            </Text>
                        )
                    }
